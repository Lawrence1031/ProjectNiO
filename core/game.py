...
class Game:
	...
	def load(self):
		...
		if len(sys.argv) > 1 and sys.argv[1]=="test" : 
			...
			arrDeck.append(Card(self.cardlist.find("007")))
			arrDeck.append(Card(self.cardlist.find("007")))
			...
			# self.set_text_scene("106")
			self.set_battle_scene("001")
	...
	def check_bgm(self):
		if self.scNow is not None and self.sBgmName!=self.scNow.sSceneName : 
	...
	def load_file(self):
		...
		if not bSave : #비정상 세이브 파일
			...
			arrDeck = []
			for c in data["arrDeck"]:
				card = Card(self.cardlist.find(c["sCode"]))
				card.bInRaid = c["bInRaid"];
				arrDeck.append(card)
			self.arrDeck = arrDeck

			arrDiscard = []
			for c in data["arrDiscard"]:
				card = Card(self.cardlist.find(c["sCode"]))
				card.bInRaid = c["bInRaid"];
				arrDiscard.append(card)
	...
	def save_setting(self):
		#배경음 효과음 전체 여부.
		data = {
			"nVolumeBgm" : self.sound.nVolumeBgm
			, "nVolumeSe" : self.sound.nVolumeSe
			, "bFullScreen" : self.bFullScreen
		}
		sJson = json.dumps(data)
	...
	def load_setting(self):
		...
		try:
			...
			self.bFullScreen = data["bFullScreen"]
			self.sound.nVolumeSe = min(1,max(0,round(data["nVolumeSe"], 2)))
			self.sound.nVolumeBgm = min(1,max(0,round(data["nVolumeBgm"], 2)))
			if self.scNow is not None : 
				self.sound.play(self.scNow.sSceneName)
	...
	def fibonacci(self, n):
		...
		if n<2 : return nNum1
		elif n==2 : return nNum2
		elif n==3 : return nNum3
	...
	def experience(self, n):
		return ((max(1,rd(n)) - 1) * 50 / 49) ** 2.5
	...
	def run(self):
		while self.bRunning:
			...
			if self.bCursorHand : pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
