...
class Stage1(Scene):
	...
	def on_Classroom1(self, place):
		...
		if place.bVisited==False : # 최초
			...
			if n>50 : 
				arrTextEvent = ["101","102"]
				sTextEvent = random.choice(arrTextEvent)
				self.game.fade_text_scene(sTextEvent)
			else :
				self.game.fade_battle_scene("101")
	...
	def on_Roof(self, place):
		...
		if place.bVisited==False : # 최초
			...
			if n>20 : # 80%
				...
				self.game.fade_battle_scene("mon013")
	...
	def on_Classroom5(self, place):
		# 확률 50%	TE	101, 102	// Base, Classroom에서 과학실, 도서실, 양호실, 방송실 제외한 Classroom들
		# 확률 50%	BE	101	이벤트 숫자가 부족하면 특수목적실 추가 하는 방향으로
