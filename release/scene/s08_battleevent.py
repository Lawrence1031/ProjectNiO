...
class Battleevent(Scene):
	...
	def check_victory(self):
		...
		for i in range(len(arrCard)-1, -1, -1):
			if arrCard[i].bBattleMelted : #휘발성 방해카드 삭제 
				arrCard.pop(i)
	...
	def be_ges(self): #게스 전투 이벤트 
		#배경 이미지 
		self.load_bg("img/bg_003.png") 
		#배경 음악
		self.load_bgm("Battleevent_3")
	...
	mon_000
	Monsterinfo("502") 1마리 (강의 로봇 약한 버전)
	총 1마리 등장
	교실(첫 튜토리얼에서만 출현)


	mon_001
	Monsterinfo("001") 2마리 (청소 로봇)
	총 2마리 등장
	교실, 계단


	mon_002
	Monsterinfo("001") 1마리 (청소 로봇)
	Monsterinfo("002") 1마리 (운반용 드론)
	총 2마리 등장
	교실, 계단
	...
	mon_009
	Monsterinfo("301") 2마리 (소방 로봇)
	총 2마리 등장
	계단


	mon_010
	Monsterinfo("302") 1마리 (과학 용품 관리 로봇)
	총 1마리 등장
	과학실(Classroom3)


	mon_011
	Monsterinfo("303") 1마리 (도서실 관리 로봇)
	총 1마리 등장
	도서실(Classroom4)


	mon_012
	Monsterinfo("501") 1마리 (강의 로봇)
	총 1마리 등장
	교실 (일정 턴 이후)


	mon_013
	Monsterinfo("503") 1마리 (선도 로봇)
	총 1마리 등장
	교실 (일정 턴 이후, 낮은 확률), 계단(일정 턴 이후, 낮은 확률), 옥상(Roof)


	mon_014
	Monsterinfo("801") 1마리 (경비 로봇)
	총 1마리 등장
	운동장(Playground)
	"""
	...
	def be_mon001(self):
		...
		#현재 위치에 따라 해당 이미지 중에서 그릴거 계산 - 부관에게 부탁

		# 2023-08-27 bk.choi 스테이지 이름 호출 함수 추가. 
		sStageName = self.game.get_stage_name()
		if sStageName=="Classroom1" : #Classroom1 이면
			self.load_bg("img/bg_001.png") # 교실
			#배경 음악
			self.load_bgm("Battleevent_1")
		elif sStageName.startswith("Classroom") : #Classroom으로 시작하면. 
			self.load_bg("img/bg_001.png") # 교실
			#배경 음악
			self.load_bgm("Battleevent_1")
		else : #그외 
	...
	def mon_010(self):
		#배경 이미지(과학실-Classroom3)
		self.load_bg("img/bg_005.png") # 과학실
		#배경 음악
		self.load_bgm("Battleevent_2")
	...
	def mon_011(self):
		#배경 이미지(도서실-Classroom4)
		self.load_bg("img/bg_006.png") # 도서실
		#배경 음악
		self.load_bgm("Battleevent_2")
	...
	def mon_012(self):
		#배경 이미지(교실)
		self.load_bg("img/bg_002.png") # 교실
		#배경 음악
		self.load_bgm("Battleevent_2")

		#일정 턴 이후에 출현되게 코딩해야됨 - 부관이에게 도움 요청
	...
	def mon_014(self):
		#배경 이미지(운동장-Playground)
		self.load_bg("img/bg_002.png") # 운동장
		#배경 음악
		self.load_bgm("Battleevent_2")
