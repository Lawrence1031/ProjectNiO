...
class Textevent(Scene):
	...
	def te_001(self):
		#배경 이미지 
		self.load_bg("img/place_classroom_01.png") 

		#시나리오 파일 읽기. 파일명 
		arrScenario = self.read_text("te_001")
	...
	def te_101(self):
		...
		#시나리오 파일 읽기. 파일명 
		arrScenario = self.read_text("te_101")
		self.set_scenario(arrScenario) #파일 분석.
		...
		def fn(nCode): #팝업창 확인.
			...
			if nCode==1 : #동료 해금
				...
				self.game.fade_battle_scene("001") #교실 몬스터와 전투
			return
		self.btOk.on_click = fn
	...
	def te_102(self):
		...
		#시나리오 파일 읽기. 파일명 
		arrScenario = self.read_text("te_102")
		self.set_scenario(arrScenario) #파일 분석.
	...
	def te_103(self):
		...
		#시나리오 파일 읽기. 파일명 
		arrScenario = self.read_text("te_103")
		self.set_scenario(arrScenario) #파일 분석.
		...
		def fn(nCode): #팝업창 확인.
			...
			if nCode==3 : #도망에 성공
				...
				self.game.fade_battle_scene("001") #복도 혹은 계단 몬스터와 전투
			return
		self.btOk.on_click = fn
	...
	def te_104(self):
		#배경 이미지 -> 양호실에 어울리는 배경으로 변경예정(성민, 경희)
		self.load_bg("img/place_classroom_01.png") 

		#시나리오 파일 읽기. 파일명 
		arrScenario = self.read_text("te_104")
		self.set_scenario(arrScenario) #파일 분석.
	...
	#과학실에서 비품을 이용하는 이벤트 
	def te_105(self):
		#배경 이미지 -> 과학실에 어울리는 배경으로 변경예정(성민, 경희)
		self.load_bg("img/place_classroom_01.png") 

		#시나리오 파일 읽기. 파일명 
		arrScenario = self.read_text("te_105")
		self.set_scenario(arrScenario) #파일 분석.
		...
		def fn(): 
			...
			if nRand <=50 : 
				...
				nCode = 2
				sText = "제조에 실패했다. 카드를 한 장 폐기한다." #카드 한장 폐기가 어색하면 피격된거로 적어도 무관
			self.pop_alert(nCode, sText)
			return
		self.bt1.on_click = fn
	...
	def te_106(self):
		#배경 이미지 -> 도서실에 어울리는 배경으로 변경예정(성민, 경희)
		self.load_bg("img/place_classroom_01.png") 

		#시나리오 파일 읽기. 파일명 
		arrScenario = self.read_text("te_106")
		self.set_scenario(arrScenario) #파일 분석.
		...
		def fn(nCode): #팝업창 확인.
			...
			#이 코드 써도 되는지 (부관에게)
			# -----------------------------------
			# if nCode==1 or 5 : #공격 카드 획득
			# 	#공격 카드(100번대) 랜덤으로 1장 획득
			# 	self.game.set_scene("Stage1")

			# elif nCode==2 or 4 : #지원 카드 획득
			# 	#지원 카드(400번대) 랜덤으로 1장 획득
			# 	self.game.set_scene("Stage1")

			# elif nCode==3 or 6 : #변화 없음
			# 	self.game.set_scene("Stage1")

			# elif nCode==7 : #피격 게이지 모두 회복
			# 	self.game.set_scene("Stage1")

			# elif nCode==8 : #회복 카드 획득
			# 	#회복 카드(200번대) 랜덤으로 1장 획득
			# 	self.game.set_scene("Stage1")
			# -----------------------------------
