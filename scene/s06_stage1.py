...
class Stage1(Scene):
	...
	def get_stage_data(self):
		...
		for place in self.groupPlace :
			...
			arrVisited[place.sName] = place.nVisited
	...
	def set_stage_data(self, stageData):
		...
		for place in self.groupPlace :
			...
			place.nVisited = stageData["arrVisited"][place.sName]
	...
	def cal_stage_step(self):
		...
		if self.game.nLevel < 2 : self.level.text("")
		else : 
			nPowerUp = 100+rd(self.game.experience(self.game.nLevel+2))
			self.level.text(str(self.game.nLevel)+"층("+str(nPowerUp)+"%)")
	...
	def update(self):
		try:
			...
			if self.btCardspace.collidepoint(pos) : self.game.bCursorHand = True
	...
	def on_Stairs3(self, place):
		...
		# 	확률 50%	BE	101	
		if place.nVisited==0 : # 최초
			self.find_place("Stairs1").nVisited += 1
			self.find_place("Stairs2").nVisited += 1
			self.find_place("Stairs3").nVisited += 1
			self.game.fade_scenario_scene("002")
		else :
			n = random.randint(1, 100) 
			if n<=50 : 
	...
	def on_Classroom8(self, place):
		...
		elif place.nVisited==1 :
			self.game.fade_text_scene("104")
		else :
			self.game.fade_text_scene("104_2")
	...
	def on_Classroom3(self, place):
		...
		if place.nVisited==0 : # 최초
			...
			self.game.fade_text_scene("105_2")
	...
	def on_Classroom4(self, place):
		...
		if place.nVisited==0 : # 최초
			...
			self.game.fade_text_scene("106_2")
