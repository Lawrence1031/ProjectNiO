...
class Textevent(Scene):
	...
	def update(self):
		try:
			...
			if( ( self.bt1.collidepoint(pos) and self.bt1.bDisplay and not self.bAlert )
				...
				or ( self.btOk.collidepoint(pos) and self.bAlert ) ): self.game.bCursorHand = True
	...
	def te_101(self):
		...
		#시나리오 이미지 
		img = pygame.image.load("img/s07_bg_001.png")
		img = pygame.transform.smoothscale(img, (300,200))
		self.set_event_img(img) #500x200 사이즈 
	...
	def te_102(self):
		...
		#시나리오 이미지 
		img = pygame.image.load("img/s07_bg_002.png")
		img = pygame.transform.smoothscale(img, (300,200))
		self.set_event_img(img) #500x200 사이즈 
	...
	def te_103(self):
		...
		#시나리오 이미지 
		img = pygame.image.load("img/s07_bg_003.png")
		img = pygame.transform.smoothscale(img, (300,200))
		self.set_event_img(img) #500x200 사이즈 
	...
	def te_104(self):
		...
		#시나리오 이미지 
		img = pygame.image.load("img/s07_bg_004.png")
		img = pygame.transform.smoothscale(img, (300,200))
		self.set_event_img(img) #500x200 사이즈 
	...
	def te_105(self):
		...
		#시나리오 이미지 
		img = pygame.image.load("img/s07_bg_005.png")
		img = pygame.transform.smoothscale(img, (300,200))
		self.set_event_img(img) #500x200 사이즈 
	...
	def te_105_2(self):
		...
		def fn(): 
			#Q1 화학 약품을 제조한다.
			#20%	카드를 획득	 공격 카드 랜덤으로 획득
			#80%	화학 약품은 위험하다. 정확한 지식없이 제조하다 다쳤다.	카드 한장 폐기
		...
		def fn(): 
			#Q3 소화기를 가져간다.
			#50%	카드를 획득	소화기 카드 혹은 비슷한 느낌의 1회성 사용 카드 획득 -> 이 쪽은 지원 카드쪽으로
			#50% 	전투
	...
	def te_106(self):
		...
		#시나리오 이미지 
		img = pygame.image.load("img/s07_bg_006.png")
		img = pygame.transform.smoothscale(img, (300,200))
		self.set_event_img(img) #500x200 사이즈 
	...
	def te_106_2(self):
		...
		def fn(): 
			#Q1 A 서고를 살펴본다.
			#20%	카드를 획득	공격 카드 랜덤 획득
			#10%	카드를 획득	지원 카드 랜덤 획득
			#20%	필요한 책을 찾지 못했다.	이벤트 종료
			#50%	전투
		...
		def fn(): 
			#Q1 B 서고를 살펴본다.
			#20%	카드를 획득	지원 카드 랜덤 획득
			#10%	카드를 획득	공격 카드 랜덤 획득
			#20%	필요한 책을 찾지 못했다.	이벤트 종료
			#50%	전투
