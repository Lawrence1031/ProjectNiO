...
class Cardshop(Scene):
	...
	def set_arr_card(self, arrCard=[]):
		...
		for i in range(0,min(len(arrCard),5)) : 
			...
			nPrice = 3
			if cardInfo.sRarity=="N" : nPrice=3
			elif cardInfo.sRarity=="A" : nPrice=9
			elif cardInfo.sRarity=="S" : nPrice=30
	...
	def load_cardshop(self, nNio):
		...
		# self.game.nRemainNio = 0 #카드샵에선 남은 니오는 건드리지 않음.
	...
	def exit_click(self):
		# self.game.nRemainNio += self.nNio
		self.game.set_scene("Deckselect")
		return
	...
	def on_mouse_button_down(self, e):
		try:
			if e.button == 1: 
				...
				if self.btExit.collidepoint(e.pos) : 
					self.game.sound_play("CURSOR")
					return self.exit_click()
		...
		try:
			...
			for div in self.gCard :
				if div.bDisplay and div.collidepoint(pos) : 
					...
					break;
			if self.text.collidepoint(pos) : self.game.bCursorHand = True
			if self.btExit.collidepoint(pos) : self.game.bCursorHand = True
