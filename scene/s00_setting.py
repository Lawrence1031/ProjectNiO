...
class Setting(Scene):
	...
	def load(self):
		...
		div.surfaceHover(surfaceBtHover)
		div.font_size(25)
		if self.game.bFullScreen : div.text("창 모드로 변경")
		else : div.text("전체 화면으로 변경")
		div.center_text(True)
		div.width(300)
		div.height(60)
		...
		try:
			if e.button == 1:
				if self.btClose.collidepoint(e.pos):
					...
					if self.game.bFullScreen : #풀스크린 -> 창모드
						self.game.screen = pygame.display.set_mode(self.game.arrWindowSize)
						self.btScreen.text("전체 화면으로 변경")
					else : #창모드 -> 풀스크린
						self.game.screen = pygame.display.set_mode(self.game.arrWindowSize, pygame.FULLSCREEN)
						self.btScreen.text("창 모드로 변경")
					self.game.bFullScreen = not self.game.bFullScreen
					self.game.save_setting()
	...
	def update(self):
		try:
			# 커서 변경.
			self.game.bCursorHand = False
			pos = pygame.mouse.get_pos()
			for div in self.groupBt :
				if div.collidepoint(pos): self.game.bCursorHand = True
