...
class Turret(pygame.sprite.Sprite):
	...
	def load_anim(self):
		...
		#설치 애니메이션 
		sprite = pygame.image.load("img/turret_001.png") #65x54
		width = 65
		height = 54
		anim = {}
		anim["arrShape"] = []
		...
		img = sprite.subsurface(pygame.Rect(width*3, 0, width, height))
		shape = self.build_shape(img, (0,0), (width,height))
		anim["arrShape"].append(shape)

		img = sprite.subsurface(pygame.Rect(width*4, 0, width, height))
		shape = self.build_shape(img, (0,0), (width,height))
		anim["arrShape"].append(shape)

		img = sprite.subsurface(pygame.Rect(width*5, 0, width, height))
		shape = self.build_shape(img, (0,0), (width,height))
		anim["arrShape"].append(shape)

		img = sprite.subsurface(pygame.Rect(width*6, 0, width, height))
		shape = self.build_shape(img, (0,0), (width,height))
		anim["arrShape"].append(shape)

		img = sprite.subsurface(pygame.Rect(width*7, 0, width, height))
		shape = self.build_shape(img, (0,0), (width,height))
		anim["arrShape"].append(shape)

		img = sprite.subsurface(pygame.Rect(width*8, 0, width, height))
		shape = self.build_shape(img, (0,0), (width,height))
		anim["arrShape"].append(shape)
