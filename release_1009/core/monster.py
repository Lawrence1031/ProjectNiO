...
class Monster(pygame.sprite.Sprite):
	...
	def set_monster_info(self, monsterinfo):
		...
		self.set_level(self.game.nLevel)
	...
	def set_level(self, nLevel):
		# nPowerUp = (1+fibonacci(nLevel)/10)
		# nPowerUp = 1+nLevel*0.5
		if nLevel > 1 : 
			nPowerUp = 1+rd(self.game.experience(nLevel+2))/100
		else : nPowerUp = 1
		print("set_level", nPowerUp*100,"%")

		self.nAD = rd(self.monsterinfo.nAD*nPowerUp)
		self.nAP = rd(self.monsterinfo.nAP*nPowerUp)
		self.nHpMax = rd(self.monsterinfo.nHp*nPowerUp)
		self.nHp = rd(self.monsterinfo.nHp*nPowerUp)
		return 
	...
	def update(self):
		...
		#버프 그리기 
		nLeft = self.nPadding+shape["offset"][0]-75+shape["rect"].width/2
		for i in range(0,len(self.arrBuff)):
			if self.arrBuff[i].icon is None : continue
			self.image.blit(self.arrBuff[i].icon, (nLeft, self.nPadding+shape["offset"][1]-55))
			nLeft += 33
